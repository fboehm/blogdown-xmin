...
Within `reticulate`, there is a function `import` to load a `python` module (ie, a *package* in the `R` terminology). But you want to be sure to store the output as something (don't just run `import()` without assigning its output to something).

```{r}
import("limmbo") -> limmbo
```

Now, we can look the object `limmbo` & its contents. Typing merely `limmbo` at the R prompt isn't terribly helpful:
